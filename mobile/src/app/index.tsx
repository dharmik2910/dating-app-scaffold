...
function parseBioContent(rawBio?: string) {
  if (!rawBio) return { cleanBio: '', interests: [], city: '' };
  let bioStr = rawBio;
  let city = '';
  ...
  return { cleanBio: bioStr, interests, city };
}
...
export default function DiscoverScreen() {
  ...
  async function handleStartChat(candidate: Candidate) {
    try {
      ...
      if (!candidate.liked) {
        await handleToggleLike(candidate);
      }
  ...
  const onlineCount = useMemo(() => {
    return candidates.filter((c) => formatUserActivity(c.lastActiveAt, c.isOnline).isOnline).length;
  }, [candidates]);
      ...
      >
        ...
        {loading ? (
                  ...
                  <View style={styles.cardBottomOverlay}>
                    ...
                    <View style={styles.grid2LocationRow}>
                      ...
                      <Text style={styles.grid2LocationText} numberOfLines={1}>
                        {city
                          ? `${city} • ${candidate.distance_km != null && candidate.distance_km > 0 ? `${candidate.distance_km.toFixed(1)} km` : 'Nearby'}`
          ...
          <View style={styles.grid1Container}>
            {filteredCandidates.map((candidate) => {
              const { cleanBio, city, interests } = parseBioContent(candidate.bio);
              const photoUrl = candidate.photos && candidate.photos[0]?.url;
              const activity = formatUserActivity(candidate.lastActiveAt, candidate.isOnline);
                ...
                >
                  <View style={styles.grid1PhotoContainer}>
                    ...
                    <View style={styles.cardBottomOverlay}>
                      ...
                      {cleanBio ? (
                        <Text style={styles.grid1BioText} numberOfLines={2}>
                          {cleanBio}
                        </Text>
                      ) : null}
                    </View>
              ...
              return (
                ...
                >
                  ...
                  <View style={styles.listTextCol}>
                    ...
                    {cleanBio ? (
                      <Text style={styles.listBioText} numberOfLines={1}>
                        {cleanBio}
                      </Text>
                    ) : null}
                  </View>
              ...
              <ScrollView contentContainerStyle={styles.modalScroll}>
                ...
                <View style={styles.modalDetailsBody}>
                  ...
                  <Text style={styles.modalLocationText}>
                    📍 {selectedCandidate.location || 'Nearby'}{' '}
